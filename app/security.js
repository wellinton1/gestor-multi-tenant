...
// Inspeção de segurança - apenas admin da plataforma
router.get('/inspect', (req, res) => {
  ...
  if (!currentUser || !isGlobalAdmin(currentUser)) {
    return res.status(403).json({ error: 'Apenas o administrador da plataforma pode acessar a inspecao de seguranca.' });
  }
  ...
  // 5. Verificar usuarios com acesso global (admin da plataforma)
  const globalAdmins = users.filter((u) => isGlobalAdmin(u));
  if (globalAdmins.length > 0) {
    issues.push({
      severity: 'info',
      category: 'Usuarios Globais',
      description: `${globalAdmins.length} usuario(s) tem acesso global a todos os estabelecimentos (admin da plataforma).`,
