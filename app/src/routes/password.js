...
router.get('/admin-users', (req, res) => {
  const currentUser = store.findById('users', req.session.userId);
  if (!currentUser || currentUser.role !== 'admin') {
    return res.status(403).json({ error: 'Apenas administradores podem gerenciar usuarios.' });
  }
...
router.put('/admin-associate/:userId', (req, res) => {
  const currentUser = store.findById('users', req.session.userId);
  if (!currentUser || currentUser.role !== 'admin') {
    return res.status(403).json({ error: 'Apenas administradores podem gerenciar associacoes.' });
  }
