...
const { resolveAllowedIds, hasAccessToEstablishment: checkStoreAccess, isGlobalAdmin } = require('../utils/access');
...
function hasAccessToEstablishment(req, establishmentId) {
  ...
  return checkStoreAccess(user, establishmentId);
}
...
router.get('/', requireEstablishment, (req, res, next) => {
  try {
    ...
    const users = store.all('users').filter((user) => {
      const allowed = resolveAllowedIds(user);
      if (allowed === null) return isGlobalAdmin(user); // acesso global: so admin
...
router.get('/by-establishment/:id', (req, res, next) => {
  try {
    ...
    // Inclui admins globais e usuarios associados a este estabelecimento.
    const users = store.all('users').filter((user) => {
      const allowed = resolveAllowedIds(user);
      if (allowed === null) return isGlobalAdmin(user);
      return Array.isArray(allowed) && allowed.includes(estId);
    });
    res.json(users.map((user) => ({
      id: user.id,
...
router.delete('/:id', requireEstablishment, (req, res, next) => {
  try {
    ...
    // Usuarios com acesso global (admins) nao podem ser removidos por estabelecimento.
