...
router.put('/config', async (req, res, next) => {
  try {
    ...
    // MantÃ©m compatibilidade com campo antigo
...
router.post('/create-customer', async (req, res, next) => {
  try {
    const config = getPaymentConfig(req.session.establishmentId);
    if (!config?.configured) return res.status(400).json({ error: 'Configure a chave da API de pagamentos nas ConfiguraÃ§Ãµes.' });

    const { email, name, cellphone, taxId } = req.body || {};
    if (!email) return res.status(400).json({ error: 'Email do cliente Ã© obrigatÃ³rio.' });
...
router.post('/create-pix', async (req, res, next) => {
  try {
    ...
    if (!amount || amount <= 0) {
      return res.status(400).json({ error: 'Valor Ã© obrigatÃ³rio e deve ser maior que zero.' });
    }
...
router.post('/create-product', async (req, res, next) => {
  try {
    ...
    if (!externalId || !name || !price) {
      return res.status(400).json({ error: 'externalId, name e price sÃ£o obrigatÃ³rios.' });
    }
